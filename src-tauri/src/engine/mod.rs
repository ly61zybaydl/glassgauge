...
fn run(app: AppHandle, hwnd: isize, mut cfg: GlassCfg, rx: Receiver<Cmd>) {
    unsafe {
                ...
                loop {
                    ...
                    loop {
                        match rx.try_recv() {
                            ...
                            Ok(Cmd::SetCfg(c)) => {
                                // 同尺寸重建效果链（σ/位移/圆角随新参数），下轮立即重渲染
                                cfg = c;
                                let p = params_for(cfg, stack.dpr);
                                if let Err(e) = stack.rend.set_geometry(
                                    stack.win.width() as u32,
                                    stack.win.height() as u32,
                                    p,
                                ) {
                                    eprintln!("engine: set cfg failed: {e}");
                                    set_mode(&app, "wallpaper");
                                    drop(stack);
                                    continue 'outer;
                                }
