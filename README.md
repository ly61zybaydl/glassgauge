...
- 用当前登录账号的令牌直接向上游 `relay.mirasim.ai` 取 `GET /v1/limits`（v0.14.0，见下）；
  拿不到令牌时回退为自动发现本机 mirasim relay（端口动态分配，扫描 + 响应形状认领）；
- 常驻展开面板：**自动显示 relay 返回的所有用量窗口**（v0.11.0，5h/7d/30d 及 `7d_fable`
  等新增窗口都会显示，窗口时长从名字解析出来算匀速线），各含用量百分比、**units 原值 · 估算
  API 费用 并排**（如 `808 / 156.8k · ≈ $4 / $784`；普通窗口 `unitsPerUsd` 默认 200 units=$1，
  Fable 窗口贵 `fableMultiplier` 默认 2.4 倍 → 480 units=$1；估算非官方）、匀速线刻度、超前/落后、重置倒计时；
...
- 账号名显示为**邮箱**：`auth.token`（`mrs1:` 密文）本机解开后是一枚 JWT，
  ...
  自动命名撞车会自动消歧而不是报错（v0.13.0：两个账号邮箱本地部分相同、只是后缀
  不同时，第二个自动附域名，如 `sam` / `sam-outlook`；只有手动输入的名字撞到
  别的账号才提示换名）。
...
- **额度改为上游直取，不再依赖本机 relay 匿名接口**（v0.14.0）：mirasim 0.0.284 起本机 relay
  变成「每个 agent 会话一个私密路径 + 会话令牌」，凭证不落盘，匿名 `GET /v1/limits` 一律 401，
  旧版挂件因此卡在「等待 Mirasim 启动」。现在挂件用同一条本机解密链解出当前账号的访问 JWT，
  直接以 Bearer 请求 `https://relay.mirasim.ai/v1/limits`（与 mirasim 自己取额度的方式一致，
  响应形状相同；`subject` 恒为当前账号，切号后没有追赶期）。本机 relay 发现保留为兜底
  （旧版 mirasim 仍可用）。令牌只在进程内使用，不下发前端、不落盘；由 mirasim 负责刷新
  （访问令牌约 1 小时过期），若 mirasim 长时间未运行导致令牌过期，面板会提示
  「登录令牌已过期 · 打开 Mirasim 刷新后自动恢复」。

### 凭证导出 / 导入 / 刷新（v0.15.0）

账号列表底部多了一排工具：**⤓ 导出**、**⤒ 导入**、**↻ 刷新令牌**；每个快照行内也有 ⤓ / ↻
两个小按钮，分别作用于该快照。

- **导出**：把当前登录（或某个快照）写成**明文 JSON**——内容就是两枚 JWT（access + refresh）
  加邮箱、套餐、到期等元数据。**拿到文件的人即可登录该账号，请自行保管。** 文件名默认
  `mirasim-<邮箱本地部分>-<日期>.json`，保存位置在原生对话框里选。
- **导入**：选一个 JSON 文件。兼容三种键名：本工具导出格式（`accessToken/refreshToken`）、
  `/auth/verify` 原始响应（`access_token/refresh_token`）、setting.json 的 auth 块
  （`token/refreshToken`，明文或本机 mrs1 密文）。导入时先向登录服务器刷新一次做验活并换到
  新令牌，再用本机 `secret.key` 封成 mrs1 落成快照；同一账号覆盖原快照，**不自动切换登录**。
  网络不通时原样入库并提示；服务端拒绝则报「凭证无效」，不入库。
- **刷新令牌**：`POST https://auth.mirasim.ai/auth/refresh {refresh_token}`，与 mirasim 自身
  的做法一致（无签名、无设备头）。新令牌写回 setting.json（当前登录，只改 token/refreshToken/exp）
  或快照文件；刷新当前登录后挂件立刻用新令牌重取一次额度。
- **对别的机器没有副作用**（2026-09-07 实测）：服务端对同一枚 refresh 幂等返回同一子代 pair，
  旧 refresh、旧 access 轮换后都仍有效，也没有重放检测和会话级吊销。所以一台机器刷新、导出、
  导入，不会让另一台掉线；导出的凭证也不会因源机继续每小时自动刷新而失效。
- 令牌全程只在 Rust 进程内流转，文件对话框（rfd）也在 Rust 侧弹出；前端只拿到路径、邮箱、
  快照名。命令行版（`cli/`）暂未提供这三项。
...
```jsonc
{
  ...
  "refreshSeconds": 60,
  "unitsPerUsd": 200,         // 普通窗口：多少 units 折 $1（估算，非官方）
  "fableMultiplier": 2.4,     // Fable 窗口单价倍数（200×2.4 = 480 units 折 $1）
  "alwaysOnTop": true,
  "glass": {
